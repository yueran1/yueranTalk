...
def get_comments():
	...
	pagination = Comment.query.order_by(Coment.timestamp.desc()).paginate(page, per_page=current_app.config['FLASKY_COMMENTS_PER_PAGE'], error_out=False)
	...
	return jsonify({
		'comments': [comment.to_jason() for comment in comments],
		'prev': prev,
		'next': next,
